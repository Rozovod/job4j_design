...
create view show_students_and_amount_with_more2_books
    as select s.name as student, count(o.book_id) as amount_books from students as s
        join orders o on s.id = o.student_id
        group by s.name
        having count(o.book_id) > 2
        order by s.name;

select * from show_students_and_amount_with_more2_books;